...
describe('Player', () => {
  ...
  test('Game works correctly when playing against an ai', () => {
    ...
    player.attack([5, 7]);
    player.attack([4, 3]);
    expect(player.getAttackedCoordinates(0)).toEqual([[0, 4], [5, 7], [4, 3]]);
  });
  ...
  test('Players can\'t attack twice onto the same coordinates and players attack by turn even on bad coordinates attack', () => {
    ...
    player.attack([1, 1]);
    player.attack([0, 8]);
    expect(player.getAttackedCoordinates(0).length).toBe(7);
    expect(player.getAttackedCoordinates(1).length).toBe(7);
  });
  ...
  test('Players can\'t attack on invalid coordinates', () => {
    ...
    expect(player.getAttackedCoordinates(0).length).toBe(2);
    expect(player.getAttackedCoordinates(1).length).toBe(2);
  });
});
